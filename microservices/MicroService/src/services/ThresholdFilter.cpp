...
std::string ThresholdFilter::getVertexShader() {
	return "#version 330 \n"
...
std::string ThresholdFilter::getFragmentShader() {
	return "#version 330 \n"
		"in vec3 col;"
		...
		"   colorOut = texColor; "
		"   if (length(colorOut.xyz) < 0.7) { discard; }"
		"   colorOut = vec4(1.0,1.0,1.0,1); "  
		"}"; 
}

void ThresholdFilter::setParameters(vislink::MessageQueue* queue, GLuint shaderProgram) {
