...
int main(int argc, char**argv) {
	if (argc < 4) {
		std::cout << "Usage: ./build/bin/SimpleClient <name> <inputTexture> <outputTexture>" << std::endl;
		exit(0);
	}

	std::string serviceName = std::string(argv[1]);
	std::string inputTextureName = std::string(argv[2]);
	std::string outputTextureName = std::string(argv[3]);
